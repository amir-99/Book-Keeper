def gitlab_retry_pipeline(project_id: str, pipeline_id: int) -> str:
    """Retry failed or canceled jobs in one GitLab pipeline.
    ...
    Returns:
        JSON containing the retried pipeline metadata, or a GITLAB_ERROR string.
    """
    import json
    import os
    ...
    request = Request(
        f"{base_url}/api/v4/projects/{encoded_project}/pipelines/{pipeline_id}/retry",
        data=b"", method="POST",
    ...
    try:
        ...
        return f"GITLAB_ERROR: GitLab returned HTTP {error.code} while retrying the pipeline"
    except (URLError, OSError, ValueError, json.JSONDecodeError) as error:
        return f"GITLAB_ERROR: unable to retry the pipeline ({type(error).__name__})"
    if not isinstance(payload, dict):
        return "GITLAB_ERROR: GitLab returned an invalid retried pipeline"

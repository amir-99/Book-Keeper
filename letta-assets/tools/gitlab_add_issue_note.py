def gitlab_add_issue_note(project_id: str, issue_iid: int, body: str, internal: bool = False) -> str:
    """Add one comment to a GitLab issue.
    ...
    request = Request(
        f"{base_url}/api/v4/projects/{encoded_project}/issues/{issue_iid}/notes",
    ...
    try:
        ...
        return f"GITLAB_ERROR: GitLab returned HTTP {error.code} while adding the issue note"
    except (URLError, OSError, ValueError, json.JSONDecodeError) as error:
        return f"GITLAB_ERROR: unable to add the issue note ({type(error).__name__})"
